...
# New library added
# Import 'Defaults' from telegram.ext for latest JobQueue configuration
...
async def send_telegram_message(context: ContextTypes.DEFAULT_TYPE, chat_id: str, message_data: dict):
    try:
        ...
        # Fallback to plain text if MarkdownV2 fails due to complex message content
        try:
...
def main():
    ...
    # FIX 1 & 2: Import 'Defaults' at the top and use this new configuration for Job Queue
    # Create the Defaults object, which enables the JobQueue by default in the Application.
    defaults = Defaults(allow_sending_messages_during_build=True)
    
    # Build the Application using the Defaults object.
    application = Application.builder().token(YOUR_BOT_TOKEN).defaults(defaults).build()
